...
- Decision: split root-first gating into two stages. Stage 1 is a hard `root/reservation compatibility gate`, not a quality filter. Stage 2 is a `capacity preflight` that estimates whether enough future release/tail-safe reservation exists before paying for full UDG/product evaluation.
