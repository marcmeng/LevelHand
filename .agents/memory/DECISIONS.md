...
- Validation boundary: gray-zone routing itself is not yet proven production-stable. Existing evidence justifies the architecture, but the first prototype must explicitly test whether `BORDERLINE -> limited UDG probe` collapses, and report `probePass/probeCollapse/probeSkipped` without feeding probe failure back into the hard compatibility gate.
